    ...
    fun storeStats(timeout: Boolean) {
        try {
            ...
            BufferedWriter(FileWriter("stats.csv", true)).use { out ->
                if (newf)
                    out.appendLine("scenario, pChange, gamma, recovery_strategy, required_certainty, build_plans, matrices, visited_states, actions, time, timeout")
                //val sNbAct = if (timeout) (nbActions+500).toDouble() else nbActions.toString()
                val sNbAct = nbActions.toString()
                out.appendLine("$scenario, ${"%.2f".format(pChange)}, ${"%.4f".format(gamma)}, $strategy, ${"%.2f".format(requiredCertainty)}, $nbPlanFor, $nbMatrices, $nbVisitedStates, $sNbAct, ${System.currentTimeMillis()-startT}, $toS")
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
